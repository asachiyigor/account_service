...
@Component
public class AccountFilterStatus implements FilterSpecification<Account, AccountDtoFilter> {
    private final static String FIELD_NAME = "status";
    ...
    public Specification<Account> apply(AccountDtoFilter filter) {
        return (root, query, cb) -> root.get(FIELD_NAME).in(filter.getStatuses());
    }
}
